...
EarthServerGenericClient.Model_WCPSDemAlpha.prototype.createModel=function(root, index, cubeSizeX, cubeSizeY, cubeSizeZ){
    ...
    if( this.WCPSString[0] === undefined || this.WCPSString[1] === undefined || this.WCPSString[2] === undefined || this.WCPSString[3] === undefined)
    {
        ...
        this.wcpsQuery += "alpha: (char) (((scale(trim(dtm , {x(" + this.minx + ":" +  this.maxx + "), y(" + this.miny + ":" + this.maxy + ') }), {x:"CRS:1"(0:' + this.XResolution + '), y:"CRS:1"(0:' + this.ZResolution + ")}, {})) / 1349) * 255)";
    ...
    else //ALL set so use custom query
    {
        ...
        for(var i=0; i<4; i++)
        {
            this.WCPSString[i] = this.WCPSString[i].replace("$CI","image");
            this.WCPSString[i] = this.WCPSString[i].replace("$CD","dtm");
            this.WCPSString[i] = this.WCPSString[i].replace("$MINX",this.minx);
            this.WCPSString[i] = this.WCPSString[i].replace("$MINY",this.miny);
            this.WCPSString[i] = this.WCPSString[i].replace("$MAXX",this.maxx);
            this.WCPSString[i] = this.WCPSString[i].replace("$MAXY",this.maxy);
            this.WCPSString[i] = this.WCPSString[i].replace("$RESX",this.XResolution);
            this.WCPSString[i] = this.WCPSString[i].replace("$RESZ",this.ZResolution);
        }
        this.wcpsQuery =  "for image in (" + this.coverageImage + "), dtm in (" + this.coverageDEM + ") return encode ( { ";
        this.wcpsQuery += "red: " + this.WCPSString[0] + " ";
        this.wcpsQuery += "green: " + this.WCPSString[1]+ " ";
        this.wcpsQuery += "blue: " + this.WCPSString[2] + " ";
        this.wcpsQuery += "alpha: " + this.WCPSString[3];
        this.wcpsQuery += '}, "' + this.imageFormat +'" )';
    }
    ...
    var data = {
        wcpsUrl:this.url,
        wcpsQuery:this.wcpsQuery,
        useWcs:false
    };
    var serverRequest = new ServerRequest(this);
    console.time(this.name+"_request");
    serverRequest.request(data);
};
/**
...
EarthServerGenericClient.Model_WCPSDemAlpha.prototype.receiveData = function( data)
{
    ...
    else
    {
        ...
        this.terrain = new Terrain(transform, data, this.index);
        this.terrain.createTerrain();
    }
};

/**
 * Updatess the transparency of the scene model. Values between 0-1 (Fully Opaque - Fully Transparent).
 * @param transparency
 */
...
EarthServerGenericClient.Model_WCPSDemWCS.prototype.createModel=function(root, index, cubeSizeX, cubeSizeY, cubeSizeZ){
    ...
    //3: Make ServerRequest and receive data.
    var lc = {};
    lc[1]= this.minx; lc[0]= this.miny;
    var uc = {};
    uc[1] = this.maxx; uc[0]= this.maxy;

    var data = {
        wcpsUrl: this.URLWCPS,
        wcpsQuery: this.wcpsQuery,
        useWcs: true,
        wcsUrl: this.URLDEM,
        wcsVersion: this.WCSVersion,
        wcsCoverID: this.coverageDEM,
        lowerCorner: lc,
        upperCorner: uc
    };
    var serverRequest = new ServerRequest(this);
